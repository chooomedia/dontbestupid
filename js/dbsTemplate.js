class DbsTemplate {
    ...
    constructor(wallet) {
        ...
        this.moneyQuotes = [{
            ...
            {
                "quote": "Only buy something that you’d be perfectly happy to hold if the market shuts down for ten years.",
            ...
            {
                "quote": "The person who doesn’t know where his next dollar is coming from usually doesn’t know where his last dollar went.",
                "author": "Unknown"
            },
            {
                ...
                "<div id='dbsOverlay'>" +
                    "<div class='inner-content clearfix'>" +
                        "<div id='question-header' class='grid' style='background: white;'>" +
                            "<h1 itemprop='name' class='grid--cell fs-headline1 fl1' style='margin: 5px auto 3px auto;position: relative;'>" +
    ...
    getRandomMoneyQuote() {
        let quotesIndex = Math.floor(Math.random() * Math.floor(this.moneyQuotes.length));
        let quote = document.createElement("q");
        quote.style.width = "67%";
        quote.style.display = "inline-block";
        quote.style.lineHeight = "22px";
        quote.innerHTML =   "<p style='font-style:italic;margin:0;'>“"+ this.moneyQuotes[quotesIndex].quote +"”</p>" +
                            "<a target='_blank' href='https://de.wikipedia.org/wiki/" + this.moneyQuotes[quotesIndex].author + "'" +
                                "title='look at " + this.moneyQuotes[quotesIndex].author + " on Wikipedia' style='color:#333;cursor:pointer;font-size:13px;margin:0;' >" +
                                this.moneyQuotes[quotesIndex].author +
                            "</a>";
        return quote;
    }

    createIntroDonateInnerDailog() {
        let donateIntroDialog = document.createElement("div");
        donateIntroDialog.id = "innerIntroDonateDialog";
        let randomMoneyQuote = this.getRandomMoneyQuote();
        donateIntroDialog.style.gridColumn = "1 / 3";
        donateIntroDialog.style.textAlign = "center";
        donateIntroDialog.style.border = "1px solid rgba(43,43,43,.3)";
        donateIntroDialog.style.marginBottom = "4px";
        donateIntroDialog.innerHTML = 
                            "<img src='http://cdn.onlinewebfonts.com/svg/img_460888.png' width='82' height='82' alt='logo'" +
                            " style='padding:8px;background:#CECECE;filter:invert(1);border-bottom-right-radius:12px;border-bottom-left-radius:12px;' />" +
                            "<h1 style='margin:0 0 4px 0;'>Your Wallet is empty...</h1>" +
                                randomMoneyQuote.outerHTML;

        let innerDialog = document.getElementsByClassName("innerDialog")[0];
        innerDialog.append(donateIntroDialog);
        return innerDialog;
    }
    ...
    createDonateThxDialog() {
        ...
        innerThxDialog.style.gridColumn = "1 / 3";
